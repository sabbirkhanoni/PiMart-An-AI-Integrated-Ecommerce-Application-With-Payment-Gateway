...
import AxiosToastError from "../../utils/AxioxToastError";
    ...
    const EditCategoryComponent = ({data : categoryData, close,fetchAllCategory}) => {
      
          const [data,setData] = useState({
            _id : categoryData._id,
            name : categoryData.name || "",
            image : categoryData.image || "No Image",
        })
    ...
    <section className="fixed top-0 left-0 bottom-0 right-0 bg-neutral-900/50 p-4 flex items-center justify-center">
      <div className="bg-white max-w-4xl w-full rounded p-4">
        ...
        <form className="grid gap-2" onSubmit={handleSubmit}>
          ...
          <button className={`${(data.name && data.image) ? "bg-blue-500 hover:bg-blue-700": "bg-gray-500 hover:bg-gray-700"} rounded-full p-1 text-white mt-5`}>{loading ? "uploading..." : "Update Category"}</button>
